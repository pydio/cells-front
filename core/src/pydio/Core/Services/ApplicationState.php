...
class ApplicationState
{
    ...
    {
        return !file_exists(PYDIO_CACHE_DIR . "/first_run_passed") &&  !file_exists(PYDIO_DATA_PATH . "/plugins/boot.conf/first_run_passed");
    }
    ...
    {
        if(!is_file(PYDIO_DATA_PATH . "/plugins/boot.conf")){
            @mkdir(PYDIO_DATA_PATH . "/plugins/boot.conf", 0666, true);
        }
        @file_put_contents(PYDIO_DATA_PATH . "/plugins/boot.conf/first_run_passed", "true");
    }
    ...
    public static function detectServerURL($withURI = false, $forceInternal = false)
    {
        ...
        if (self::sapiIsCli()) {
            Logger::debug("WARNING, THE FRONTEND_URL IS NOT SET, WE CANNOT BUILD IT WHEN WORKING IN CLI");
            return "";
        }
    ...
    public static function getWorkspaceShortcutURL($repository)
    {
        ...
        $skipHistory = ConfService::getGlobalConf("SKIP_USER_HISTORY", "auth");
        if ($skipHistory) {
            $prefix = "/ws-";
        } else {
            $prefix = "?goto=";
    ...
    /**
     * Try to load the tmp dir from the CoreConf PYDIO_TMP_DIR, or the constant PYDIO_TMP_DIR,
    ...
    {
        $conf = ConfService::getGlobalConf("PYDIO_TMP_DIR");
        if (!empty($conf)) {
            return $conf;
        }
        if (defined("PYDIO_TMP_DIR") && PYDIO_TMP_DIR != "") {
            return PYDIO_TMP_DIR;
        }
