...
class LocaleService
{
    ...
    public static function getLanguage()
    {
        ...
        if ($lang == null) {
            $lang = ConfService::getInstance()->getGlobalConf("DEFAULT_LANGUAGE", "conf");
        }
    ...
    {
        $i18nFiles = glob(dirname(PYDIO_PLUGINS_MESSAGES_FILE) . "/i18n/*.ser");
    ...
    private function getMessagesInst($forceRefresh = false)
    {
        ...
        $messageCacheDir = dirname(PYDIO_PLUGINS_MESSAGES_FILE)."/i18n";
        $messageFile = $messageCacheDir."/".$crtLang."_".basename(PYDIO_PLUGINS_MESSAGES_FILE);
        ...
        if (!isset($this->cache["MESSAGES"]) && is_file($messageFile)) {
            ...
            foreach ($nodes as $node) {
                ...
                $path = PYDIO_INSTALL_PATH."/".$node->getAttribute("path");
                $remote = $node->getAttribute("remote");
                $lang = $crtLang;
                if(!empty($remote)){
                    $bootConf = ConfService::bootstrapCoreConf("ENDPOINT_FRONT_PLUGINS");
                    if(!empty($bootConf)){
