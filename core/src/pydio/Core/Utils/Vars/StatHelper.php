...
class StatHelper
{
     ...
     * @return array
     */
    public static function getMimeInfo($node, $isDir = null)
    {
        if ($isDir === null) {
            $isDir = !$node->isLeaf();
        }
        $fileName = strtolower($node->getPath());
        $registeredExtensions = self::getRegisteredExtensions($node->getContext());
    ...
    /**
     * Get all registered extensions, from the plugins
     * @static
     ...
     * @return string
     */
    public static function getPydioMimes($keyword)
    {
        if ($keyword == "editable") {
            // Gather editors!
    ...
    {
        return preg_match("/\.zip$/i", $fileName)|| preg_match("/\.tar$/i", $fileName)|| preg_match("/\.tar\.gz$/i", $fileName);
    }
