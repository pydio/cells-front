...
class LocaleExtractor
{
    ...
    public static function extractConfStringsFromManifests()
    {
        ...
        foreach ($plugins as $pType => $plugs) {
            foreach ($plugs as $plug) {
                ...
                $refFile = PYDIO_INSTALL_PATH . "/" . $path . "/conf/en.php";
                $reference = array();
                ...
                if ($namespace == "") {
                    $mixXml = file_get_contents(PYDIO_INSTALL_PATH . "/" . PYDIO_PLUGINS_FOLDER . "/core.pydio/mixins.xml");
    ...
    public static function updateI18nFiles($baseDir, $detectLanguages = true, $createLanguage = "")
    {
        ...
        if ($createLanguage != "" && !is_file($baseDir . "/$createLanguage.php")) {
            @copy(PYDIO_INSTALL_PATH . "/plugins/core.pydio/i18n-template.php", $baseDir . "/$createLanguage.php");
        }
        if (!$detectLanguages) {
