...
class DocsParser
{
    ...
    {
        return PYDIO_INSTALL_PATH . "/" . PYDIO_DOCS_FOLDER . "/hooks.json";
    }
    ...
    public static function extractHooksToDoc()
    {
        ...
        $allPhpFiles1 = self::glob_recursive(PYDIO_BIN_FOLDER . "/*.php");
        $allPhpFiles2 = self::glob_recursive(PYDIO_INSTALL_PATH . "/plugins/*.php");
        $allPhpFiles3 = self::glob_recursive(PYDIO_INSTALL_PATH . "/conf/*.php");
            ...
            foreach ($fileContent as $lineNumber => $line) {
                if (preg_match_all('/Controller::applyHook\("([^"]+)", (.*)\)/', $line, $matches)) {
                    ...
                    foreach ($names as $index => $hookName) {
                        ...
                        $filename = substr($phpFile, strlen(PYDIO_INSTALL_PATH));
