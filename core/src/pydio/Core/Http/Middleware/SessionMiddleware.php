...
class SessionMiddleware
{
    ...
    public static function handleRequest(\Psr\Http\Message\ServerRequestInterface $requestInterface, \Psr\Http\Message\ResponseInterface $responseInterface, callable $next = null){
        ...
        if(defined("PYDIO_SESSION_HANDLER_PATH") && defined("PYDIO_SESSION_HANDLER_CLASSNAME") && file_exists(PYDIO_SESSION_HANDLER_PATH)){
            require_once(PYDIO_SESSION_HANDLER_PATH);
            if(class_exists(PYDIO_SESSION_HANDLER_CLASSNAME, false)){
                $sessionHandlerClass = PYDIO_SESSION_HANDLER_CLASSNAME;
                /** @var \Pydio\Enterprise\Session\PydioSessionHandler $sessionHandler */
