...
class SessionRepositoryMiddleware
{
    ...
    public static function handleRequest(ServerRequestInterface $requestInterface, ResponseInterface $responseInterface, callable $next = null) {
        ...
        } else if(isSet($requestInterface->getCookieParams()["PYDIO_lang"])) {
            LocaleService::setLanguage($requestInterface->getCookieParams()["PYDIO_lang"]);
    ...
    public static function switchUserToRepository(UserInterface $user, ServerRequestInterface $requestInterface) {
        ...
        $repoObject         = null;
        $accessList         = RepositoryService::contextUserRepositories($user);

        if($restRepositoryId !== null){

            $repoObject = $accessList->workspaceById($restRepositoryId);
            ...
            if($repoId !== null){
                try{
                    $repoObject = $accessList->workspaceById($repoId);
                }catch (WorkspaceForbiddenException $e){
                    $previous = SessionService::getPreviousRepositoryId();
                    if($previous !== null){
                        $repoObject = $accessList->workspaceById($previous);
                    }
                }
            }else{
                $userRepositories = $accessList->getWorkspaces();
                if(empty($userRepositories)){
                    throw new NoActiveWorkspaceException();
                }
                $pendingId = SessionService::checkPendingRepository($user);
                $default = $user->getMergedRole()->filterParameterValue("core.conf", "DEFAULT_START_REPOSITORY", PYDIO_REPO_SCOPE_ALL, -1);
                ...
                }else if($default !== -1 && $default != "pydiogateway" && array_key_exists($default, $userRepositories)){
                    $repoObject = $userRepositories[$default];
                }else if($lastVisited !== "" && $default != "pydiogateway" && array_key_exists($lastVisited, $userRepositories)){
                    $repoObject = $userRepositories[$lastVisited];
                }else{
                    $repoObject = array_shift($userRepositories);
                    if ($repoObject->getId() === "pydiogateway") {
                        $repoObject = array_shift($userRepositories);
                    }
                }
            }
