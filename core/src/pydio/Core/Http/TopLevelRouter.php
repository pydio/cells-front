...
class TopLevelRouter
{
    ...
    public function __construct($cacheOptions = []){
        $this->cacheOptions = array_merge([
            "cacheDisabled" => PYDIO_SKIP_CACHE,
            "cacheFile" => PYDIO_CACHE_DIR."/".TopLevelRouter::ROUTE_CACHE_FILENAME
        ], $cacheOptions);
    }
    ...
    public function configureRoutes($base, RouteCollector &$r){
        ...
        $file = PYDIO_INSTALL_PATH."/".PYDIO_PLUGINS_FOLDER."/core.pydio/routes/main.json";
        $textContent = file_get_contents($file);
        $textContent = str_replace("%PUBLIC_BASEURI%", ConfService::getGlobalConf("PUBLIC_BASEURI", "conf"), $textContent);
    ...
    public function route(){
        ...
        switch ($routeInfo[0]) {
            case Dispatcher::FOUND:
                $data = $routeInfo[1];
                if(isSet($data["path"])){
                    require_once (PYDIO_INSTALL_PATH."/".$data["path"]);
                }
            ...
            default:
                ...
                echo file_get_contents(PYDIO_INSTALL_PATH . "/plugins/gui.ajax/res/html/404.html");
                die();
        }
