...
class PluginsService
{
    ...
    public static function clearPluginsCache(){
        ...
        @unlink(PYDIO_PLUGINS_QUERIES_CACHE);
        @unlink(PYDIO_PLUGINS_BOOTSTRAP_CACHE);
        @unlink(PYDIO_CACHE_DIR."/".TopLevelRouter::ROUTE_CACHE_FILENAME);
        if(@unlink(PYDIO_PLUGINS_REPOSITORIES_CACHE)){
            $content = "<?php \n";
            ...
            @file_put_contents(PYDIO_PLUGINS_REPOSITORIES_CACHE, $content);
        }
    }
    ...
    public static function clearRegistryCaches(){
        foreach(self::$instances as $instance){
            CacheService::delete(PYDIO_CACHE_SERVICE_NS_SHARED, $instance->getRegistryCacheKey(true));
            CacheService::delete(PYDIO_CACHE_SERVICE_NS_SHARED, $instance->getRegistryCacheKey(false));
        }
    }
    ...
    private function initRepositoryPlugins($ctx){
        ...
        try {
            ...
            throw new RepositoryLoadException($repository, [$e->getMessage()]);
        }
    ...
    public function getFilteredXMLRegistry($extendedVersion = true, $clone = false, $useCache = false)
    {
        if ($useCache) {
            ...
            $cachedXml = CacheService::fetchWithTimestamps(PYDIO_CACHE_SERVICE_NS_SHARED, $cacheKey, $tStamps);
            if ($cachedXml !== false) {
        ...
        if ($useCache && isSet($cacheKey)) {
            CacheService::saveWithTimestamp(PYDIO_CACHE_SERVICE_NS_SHARED, $cacheKey, $registry->saveXML());
        }
    ...
    private function loadPlugins($pluginFolder, $confStorage, $cacheStorage)
    {
        ...
        if(!empty($root)) {
            ...
            if(!empty($json)){
                ...
                if(!empty($data)){
                    foreach ($data as $remotePlugin){
                        $plugin = new Plugin($remotePlugin, $root."/".$remotePlugin);
                        $plugin->loadManifest();
                        if($plugin->manifestLoaded()){
                            $pluginsPool[$plugin->getId()] = $plugin;
                        }
        ...
        if (!defined("PYDIO_SKIP_CACHE") || PYDIO_SKIP_CACHE === false) {
            FileHelper::saveSerialFile(PYDIO_PLUGINS_REQUIRES_FILE, $this->required_files, false, false);
            FileHelper::saveSerialFile(PYDIO_PLUGINS_CACHE_FILE, $this->detectedPlugins, false, false);
            if (is_file(PYDIO_PLUGINS_QUERIES_CACHE)) {
                @unlink(PYDIO_PLUGINS_QUERIES_CACHE);
            }
    ...
    public function softLoad($pluginId, $pluginOptions)
    {
        ...
        $plugin = new Plugin($pluginId, PYDIO_INSTALL_PATH."/".PYDIO_PLUGINS_FOLDER."/".$pluginId);
        $plugin->loadManifest();
    ...
    {
        if(PYDIO_SKIP_CACHE) return null;
        $test = FileHelper::loadSerialFile(PYDIO_PLUGINS_QUERIES_CACHE);
    ...
    {
        if(PYDIO_SKIP_CACHE) return;
        $test = FileHelper::loadSerialFile(PYDIO_PLUGINS_QUERIES_CACHE);
        if(!is_array($test)) $test = [];
        $test[$key] = $value;
        FileHelper::saveSerialFile(PYDIO_PLUGINS_QUERIES_CACHE, $test);
    }
    ...
    public function patchPluginWithMixin(&$plugin, &$manifestDoc, $mixinName)
    {
        ...
        if (!isSet($this->mixinsDoc)) {
            ...
            $this->mixinsDoc->load(PYDIO_INSTALL_PATH."/".PYDIO_PLUGINS_FOLDER."/core.pydio/mixins.xml");
    ...
    private function savePluginsRegistryToCache($cacheStorage) {
        if (!empty ($cacheStorage)) {
            $cacheStorage->save(PYDIO_CACHE_SERVICE_NS_SHARED, "plugins_registry", $this->detectedPlugins);
        }
    }

    /**
     * Go through all plugins and call their getRegistryContributions() method.
     * Add all these contributions to the main XML pydio_registry document.
    ...
    private function buildXmlRegistry($extendedVersion = true)
    {
        ...
        $reg = new \DOMDocument();
        $reg->loadXML("<pydio_registry></pydio_registry>");
    ...
    private function recursiveLoadPlugin(AbstractConfDriver $confStorage, $plugin, $pluginsPool)
    {
        ...
        if($plugin->isEnabled($this->context) || (isSet($options["PYDIO_PLUGIN_ENABLED"]) && $options["PYDIO_PLUGIN_ENABLED"] === true)){
            $plugin = $this->instanciatePluginClass($plugin);
        }
    ...
    private function _loadDetectedPluginsFromCache($cacheStorage){
        ...
        if((!defined("PYDIO_SKIP_CACHE") || PYDIO_SKIP_CACHE === false)){
            $reqs = FileHelper::loadSerialFile(PYDIO_PLUGINS_REQUIRES_FILE);
            if (count($reqs)) {
                ...
                if (!empty($cacheStorage)) {
                    $res = $cacheStorage->fetch(PYDIO_CACHE_SERVICE_NS_SHARED, 'plugins_registry');
                    if(is_array($res)){
                ...
                if (empty($res) || !is_array($res)) {
                    $res = FileHelper::loadSerialFile(PYDIO_PLUGINS_CACHE_FILE);
                    $this->detectedPlugins=$res;
                    $this->savePluginsRegistryToCache($cacheStorage);
                }
    ...
    private function instanciatePluginClass($plugin)
    {
        ...
        $filename = PYDIO_INSTALL_PATH."/".$definition["filename"];
    ...
    private function filterRegistryFromRole(&$registry, ContextInterface $ctx)
    {
        ...
        $crtRepoId = PYDIO_REPO_SCOPE_ALL; // "ajxp.all";
        ...
        foreach ($parameters as $scope => $paramsPlugs) {
            if ($scope === PYDIO_REPO_SCOPE_ALL || $scope === $crtRepoId || ($crtRepo != null && $crtRepo->hasParent() && $scope === PYDIO_REPO_SCOPE_SHARED)) {
    ...
    private function __construct($ctx = null, $pluginsDir = null)
    {
        $this->context = $ctx;
        if(empty($pluginsDir)){
            $pluginsDir = PYDIO_INSTALL_PATH.DIRECTORY_SEPARATOR.PYDIO_PLUGINS_FOLDER;
        }
