            ...
            case "get_bookmarks":
                ...
                if (isSet($httpVars["bm_action"]) && isset($httpVars["bm_path"])) {
                    ...
                    if ($httpVars["bm_action"] == "add_bookmark") {
                        ...
                        if ($driver) {
                            $node = new Node($ctx->getUrlBase().$bmPath);
                            $node->setMetadata("ajxp_bookmarked", ["ajxp_bookmarked" => "true"], true, PYDIO_METADATA_SCOPE_REPOSITORY, true);
                        }
                    } else if ($httpVars["bm_action"] == "delete_bookmark") {
                        $bmUser->removeBookmark($repositoryId, $bmPath);
                        if ($driver) {
                            $node = new Node($ctx->getUrlBase().$bmPath);
                            $node->removeMetadata("ajxp_bookmarked", true, PYDIO_METADATA_SCOPE_REPOSITORY, true);
                        }
