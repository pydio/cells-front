...
class doctrineCacheDriver extends AbstractCacheDriver
{
     ...
     */
    public function getCacheDriver($namespace = PYDIO_CACHE_SERVICE_NS_SHARED){
    ...
    private function initCacheWithNamespace($namespace){
        ...
        switch ($driverOptions['driver']) {
            case "array":
                $cacheDriver = $this->_array_init($driverOptions);
                break;
            case "apc":
        ...
        if ($namespace == PYDIO_CACHE_SERVICE_NS_NODES) {
    ...
    /**
     * @param $options
     * @return Ext\PydioArrayCache
     */
    public function _array_init($options) {
        return new Ext\PydioArrayCache();
    }
